...
describe("reviewFees", () => {
  it("flags a doc fee above a hard-cap state's cap as critical", () => {
    ...
    expect(a.messages.some((m) => /above known state cap/i.test(m.title))).toBe(true);
    expect(a.lineItems[0].category).toBe("doc_fee");
  });

  it("does NOT raise a critical for a doc fee in an uncapped state", () => {
    const a = reviewFees([{ label: "Doc fee", amount: 700 }], "FL");
    expect(severities(a)).not.toContain("critical");
    expect(severities(a)).toContain("warning"); // still worth reviewing
  });
